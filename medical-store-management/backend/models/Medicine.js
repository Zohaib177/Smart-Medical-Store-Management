...
class Medicine extends BaseModel {
  ...
  async getPosMedicines(options={}) { const {page,limit}=normalizePagination(options.page,options.limit);const offset=calculateOffset(page,limit);const where=["m.status='active'","m.stock_quantity>0","(m.expiry_date IS NULL OR m.expiry_date>=CURRENT_DATE)"];const values=[];if(options.search){const s=`%${String(options.search).trim()}%`;where.push('(m.medicine_name LIKE ? OR m.generic_name LIKE ? OR m.barcode LIKE ? OR m.batch_number LIKE ?)');values.push(s,s,s,s);}if(options.categoryId){where.push('m.category_id=?');values.push(options.categoryId);}const ws=`WHERE ${where.join(' AND ')}`;const fields="m.id,m.medicine_name,m.generic_name,m.barcode,m.batch_number,m.strength,m.dosage_form,m.selling_price,m.stock_quantity,m.expiry_date,m.image,m.prescription_required,mc.id category_id,mc.category_name,co.id company_id,co.company_name,CASE WHEN m.expiry_date IS NULL THEN NULL ELSE DATEDIFF(m.expiry_date,CURRENT_DATE) END days_until_expiry";const joins='JOIN medicine_categories mc ON mc.id=m.category_id JOIN medicine_companies co ON co.id=m.company_id';const rows=await executeQuery(`SELECT ${fields} FROM medicines m ${joins} ${ws} ORDER BY m.medicine_name ASC LIMIT ${limit} OFFSET ${offset}`,values);const count=await executeSingle(`SELECT COUNT(*) totalRecords FROM medicines m ${joins} ${ws}`,values);return{data:rows.map(r=>({id:r.id,medicineName:r.medicine_name,genericName:r.generic_name,barcode:r.barcode,batchNumber:r.batch_number,strength:r.strength,dosageForm:r.dosage_form,salePrice:Number(r.selling_price||0),currentStock:Number(r.stock_quantity||0),expiryDate:r.expiry_date,daysUntilExpiry:r.days_until_expiry==null?null:Number(r.days_until_expiry),expiryStatus:r.days_until_expiry==null?'unknown':Number(r.days_until_expiry)<=30?'expiringSoon':'valid',image:r.image,prescriptionRequired:Boolean(r.prescription_required),category:{id:r.category_id,categoryName:r.category_name},company:{id:r.company_id,companyName:r.company_name}})),pagination:buildPaginationMetadata(page,limit,Number(count?.totalRecords||0))}; }
  async getSaleCategories(){return executeQuery("SELECT DISTINCT mc.id,mc.category_name AS categoryName FROM medicine_categories mc JOIN medicines m ON m.category_id=mc.id WHERE mc.status='active' AND m.status='active' AND m.stock_quantity>0 AND (m.expiry_date IS NULL OR m.expiry_date>=CURRENT_DATE) ORDER BY mc.category_name");}
